...
(function() {
  ...
  var template = document.getElementById("tempdom-template").innerText;

  var allData = JSON.parse(document.getElementById("tempdom-data").innerText);

  
  utils.template = template;
  
  utils.allData = allData;

  var dataDom = document.getElementById('tempdom-data');
  dataDom.parentNode.removeChild(dataDom)
  var tplDom = document.getElementById('tempdom-template');
  tplDom.parentNode.removeChild(tplDom)
  ...
  function detailReander(tableName, allData) {
    ...
    for (var i = 0; i < dpriceDetailData.length; i++) {
      var className =
        i == dpriceDetailData.length - 1 ? "detail-content-tr" : "";
  ...
  function tableRnder(type, allData) {
    ...
    var housHander =
      '<tr class="bg-big"><td style="text-align:center;width:145px">服务期</td><td style="width:150px">服务费</td><td style="text-align:center;width:170px">单价</td><td style="text-align:center">数量</td><td style="text-align:center">小计</td></tr>';
    var housContent =
      '<tr><td class="date-format" style="width:145px">{{start_date}}至<p>{{end_date}}</p></td><td style="width:150px">{{calculat_descr}}</td><td style="text-align:right;width:170px">{{unit_price}}</td><td>{{count}}</td><td style="text-align:right">{{amount}}</td></tr>';
    var stationHander =
      '<tr class="bg-big"><td style="text-align:center;width:145px">服务期</td><td style="width:150px">服务费</td><td style="text-align:center;width:170px">单价</td><td style="text-align:center">数量</td><td style="text-align:center">小计</td></tr>';
    var stationContent =
      '<tr><td class="date-format" style="width:145px">{{start_date}}至<p>{{end_date}}</p></td><td style="width:150px">{{calculat_descr}}</td><td style="text-align:right;width:170px">{{unit_price}}</td><td>{{count}}</td><td style="text-align:right">{{amount}}</td></tr>';
    var footer =
      '<div class="total"><div style="text-align:left;width:70%"><div style="word-break:break-all;width:100%"><div class="total-box wrapDiv">{{station_type}}</div><div class="total-box wrapDiv longWrap"><div class="cn_and_en">{{code_or_name}}</div></div></div></div><div style="text-align:right;width:30%">合计:￥{{total_amount}}</div></div>';
    var tableHtml =
      spliceKey(footer, allData) + ' <table cellspacing="0" >' + housHander;
    if (type == "station") {
      tableHtml =
        spliceKey(footer, allData) +
        ' <table cellspacing="0" >' +
        stationHander;
    }
  ...
  function tagRender(template, data) {
    ...
    //二维码
    var imgLabelling = allData.cachetUrl
      ? '<img style="position:absolute;display:inline-block;width:160px;height:160px;left:-80px;top:-80px" src = "' +
        allData.cachetUrl +
        '">'
      : "";
    template = template.replace(
      imgReg,
      '<span class="print-other-chapter" style="position: relative;">' +
        imgLabelling +
        "</span>"
    );
    template = template.replace(
      pageReg,
      '<div class = "print-pagination-avoid"></div>'
    );
