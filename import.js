 ...
 * @LastEditTime: 2018-12-05 10:37:05
...
function injectDir(dir) {
  ...
  if (stats.isDirectory()) {
    return readdirPromisify(dir)
      .then(list =>
        Promise.all(
          list.map(item => injectDir(path.resolve(dir, item)))
        )
      )
...
function injectVueFile(filePath) {
  ...
  fs.readFile(filePath, { flag: "rs+", encoding: "utf8" }, function(err, data) {
    ...
    // 判断是否有引用iview组件
    const reg = /(?<=<)(([A-Z]|i-)(\w|-)+\b)(?=.+template)/sgm; //只能匹配template里的以大写字母获取i-开头的标签
    if (reg.test(data)) {
      // 获取引用的组件
      let componentsArr=data.match(reg)//两种特殊情况：1，含有自定义组件 需要去掉 ；2，类似Input需要改为Input Form-item改为FormItem
      ...
      if (components.length === 0) {
        console.log('\x1B[33m 无需写入: \x1B[39m',filePath);
        return;
      }
